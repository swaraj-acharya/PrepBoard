...
// Saved solutions (lib/solutions.js): counts only; the code and reviews are in solutions/.
function notebookLine(nb) {
  if (!nb?.questions) return "";
  return `\nSolution notebook: **${nb.questions}** question${nb.questions === 1 ? "" : "s"} with saved solutions (${nb.attempts} attempt${nb.attempts === 1 ? "" : "s"}), ${nb.reviewed} with an AI review. Each question's attempts and reviews are in [solutions/](solutions/).\n`;
}

export function buildReadme(s, data, notebook) {
...
${notebookLine(notebook)}${ratingLines(s)}
