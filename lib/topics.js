...
export const TOPICS = {
  ...
  "Scalability": {
    ...
    idea: "Vertical scaling means a bigger machine: easy, but it has a limit and one failure point. Horizontal scaling means more machines behind a load balancer, which needs servers that don't store user state.",
  ...
  "Estimation": {
    ...
    idea: "Estimate users, requests per second, storage per day and bandwidth. Handy facts: one day ≈ 86,400 seconds ≈ 10^5. A million requests a day ≈ 12 per second.",
  ...
  "Load Balancer": {
    ...
    idea: "It spreads requests across servers (round robin, least connections, hashing) and stops sending traffic to a dead server (health checks).",
  ...
  "Caching": {
    ...
    idea: "Keep popular data in fast memory (Redis, Memcached) close to the app. Decide what to throw out when it's full (LRU), how long things live (TTL), and how to keep it fresh (cache-aside, write-through).",
  ...
  "Sharding": {
    ...
    idea: "Split data across many databases using a shard key (user id, region). Pick a key that spreads the load evenly. Queries that need several shards become harder.",
  ...
  "Replication": {
    ...
    idea: "Keep copies of the data on several machines. A leader takes the writes and followers serve reads. It improves safety and read speed, but copies can be slightly behind (replication lag).",
  ...
  "Message Queue": {
    ...
    idea: "Producers put messages in a queue (Kafka, RabbitMQ, SQS) and consumers handle them later. It separates services, absorbs traffic spikes, and makes retries easy.",
  ...
  "API Design": {
    ...
    idea: "Define clear endpoints (REST like GET /users/{id}, or gRPC), what goes in and comes out, errors, pagination and login. Make write APIs idempotent so retrying is safe.",
  ...
  "Microservices": {
    ...
    idea: "Split the app into small services that each own their data and talk through APIs or queues. Each can scale and ship on its own, but debugging and keeping data consistent gets harder.",
