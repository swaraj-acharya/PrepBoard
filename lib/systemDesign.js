...
// The Boring Education's free system design guides. The roadmap in lib/systemDesignPath.js follows their
// phase order, and its interview map follows their Top 30 list; the answers stay on their site.
export const TBE_ROADMAP = { name: "System Design Engineer Roadmap (The Boring Education)", url: "https://resources.theboringeducation.com/resources/system-design-roadmap" };
export const TBE_QA = { name: "Top 30 System Design Interview Questions (The Boring Education)", url: "https://resources.theboringeducation.com/resources/system-design-interview-questions" };
export const tbe = n => ({ platform: `The Boring Education, Q${n}`, url: TBE_QA.url });
...
export const HLD_GROUPS = [
  ...
  { id: "hld-hard", name: "Hard", topics: ["Geospatial Index", "Real-time Connections", "Transactions and Locking"], items: [
    ...
    h("flash-sale", "Design checkout for a flash sale that never oversells", "H", ["Transactions and Locking", "Idempotency", "Caching", "Message Queue"], { platform: "The Boring Education", url: TBE_QA.url }, { ...TBE_QA, name: `${TBE_QA.name}, Q29` }),
  ]},
];
...
// The Boring Education's Top 30 answers these designs: linked as extra reading, by question number.
const TBE_MORE = {
  "hld:load-balancer": 3, "hld:cdn": 7, "hld:rate-limiter": 11, "hld:id-generator": 20, "hld:url-shortener": 21, "hld:kv-store": 22,
  "hld:whatsapp": 23, "hld:facebook": 24, "hld:uber": 25, "hld:youtube": 26, "hld:netflix": 26, "hld:dropbox": 27, "hld:monitoring": 28,
};
// Newer explanations (lib/topics.js) that these questions also depend on. Added after the existing concepts,
// so each question's main idea stays first.
const MORE_CONCEPTS = {
  "hld:cdn": ["Latency and Throughput"],
  "hld:payment": ["Idempotency"], "hld:wallet": ["Idempotency"], "hld:upi": ["Idempotency"], "hld:payment-gateway": ["Idempotency"],
  "hld:notification": ["Idempotency"], "hld:job-scheduler": ["Idempotency"], "hld:webhooks": ["Idempotency", "Fault Tolerance"],
  "hld:kafka": ["Event-Driven Architecture"], "hld:ecommerce": ["Event-Driven Architecture"], "hld:order-management": ["Event-Driven Architecture"],
  "hld:kv-store": ["Consistency Models"], "hld:dynamodb": ["Consistency Models"], "hld:google-docs": ["Consistency Models"],
  "hld:multi-dc": ["Consistency Models", "Fault Tolerance"], "hld:cassandra": ["Consistency Models", "Bloom Filter"],
  "hld:spanner": ["Consensus and Leader Election", "Consistency Models"], "hld:lock-service": ["Consensus and Leader Election"],
  "hld:zookeeper": ["Consensus and Leader Election"], "hld:chubby": ["Consensus and Leader Election"],
  "hld:bigtable": ["Bloom Filter"], "hld:web-crawler": ["Bloom Filter"],
  "hld:netflix": ["Circuit Breaker"], "hld:api-aggregator": ["Circuit Breaker"], "hld:deploy": ["Fault Tolerance"],
  "hld:monitoring": ["Observability"], "hld:analytics": ["Observability"], "hld:zipkin": ["Observability"], "hld:dapper": ["Observability"], "hld:log-search": ["Observability"],
  "lld:text-editor": ["Command"], "lld:notification-sender": ["Adapter"], "lld:bloom-filter": ["Bloom Filter"],
};
for (const g of [...HLD_GROUPS, ...LLD_GROUPS]) for (const q of g.items) {
  if (EXTRA[q.id]) q.more = [...q.more, ...EXTRA[q.id]];
  if (TBE_MORE[q.id]) q.more = [...q.more, tbe(TBE_MORE[q.id])];
  if (MORE_CONCEPTS[q.id]) q.concepts = [...q.concepts, ...MORE_CONCEPTS[q.id].filter(c => !q.concepts.includes(c))];
  SD_BY_ID[q.id] = q;
}
