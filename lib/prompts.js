...
// Some AIs answer long prompts with a downloadable file, canvas or artifact, and tables or heavy
// Markdown come out jumbled when copied. So the reply is asked for as a plain chat message that
// still reads well after it's copied and pasted into Prepboard.
const FORMAT = `How to write your reply:
- Reply right here in the chat, as a normal message. Don't create a file, document, canvas or artifact, and don't offer a download.
- I'll copy your whole reply and paste it into a plain text box, so keep the layout simple and easy to copy: put each numbered part's title on its own line in capitals (like "1. VERDICT"), use "- " for points, and leave a blank line between parts.
- Put every piece of code in its own code block, with nothing else inside it.
- Don't use tables, bold or italic text, # headings or emojis.`;
